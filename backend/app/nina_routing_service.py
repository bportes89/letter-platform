...
def assess(db:Session,user:User,proposal:Proposal,policy:NinaRoutingPolicy,*,asset_type:str,municipality_code:str,population:int,income_per_capita:Decimal,encumbrances:list[str],risk_flags:list[str],tapaf_evidence_reference:str|None)->NinaRoutingAssessment:
    ...
    if risks and asset!="REAL_ESTATE":blockers.append("RISK_RESCUE_REQUIRES_REAL_ESTATE")
    if proposal.product=="FLASH_CREDIT" and asset!="REAL_ESTATE":blockers.append("FLASH_CREDIT_REAL_ESTATE_ONLY")
    product_route="BLOCKED" if blockers else ("FLASH_CREDIT" if risks or proposal.product=="FLASH_CREDIT" else "CONSORTIUM_MARKETPLACE")
    capital_route=None
