...
class User(TimestampMixin, Base):
    ...
    company_cnpj: Mapped[str | None] = mapped_column(String(20))
    company_address: Mapped[str | None] = mapped_column(String(240))
    company_city: Mapped[str | None] = mapped_column(String(120))
...
class UserContractAcceptance(TimestampMixin, Base):
    __tablename__ = "user_contract_acceptances"
    __table_args__ = (UniqueConstraint("user_id", "template_slug", name="uq_user_contract_acceptance"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uid)
    organization_id: Mapped[str] = mapped_column(ForeignKey("organizations.id"), index=True)
    user_id: Mapped[str] = mapped_column(ForeignKey("users.id"), index=True)
...
class QuotaSellOffer(TimestampMixin, Base):
    ...
    inventory_quota_id: Mapped[str | None] = mapped_column(ForeignKey("quotas.id"), index=True)
...
class EscrowAccount(TimestampMixin, Base):
    ...
    asaas_subaccount_api_key: Mapped[str | None] = mapped_column(Text)
    subaccount_name: Mapped[str | None] = mapped_column(String(180))
    bank_code: Mapped[str | None] = mapped_column(String(10))
    bank_agency: Mapped[str | None] = mapped_column(String(20))
    bank_account_number: Mapped[str | None] = mapped_column(String(40))
    pix_key: Mapped[str | None] = mapped_column(String(180))
    asaas_kyc_status: Mapped[str | None] = mapped_column(String(40))
    asaas_commercial_status: Mapped[str | None] = mapped_column(String(40))
    asaas_onboarding_url: Mapped[str | None] = mapped_column(Text)
...
class PreAnalysisPauta(TimestampMixin, Base):
    ...
    asset_type: Mapped[str] = mapped_column(String(40), default="REAL_ESTATE")
    asaas_payment_id: Mapped[str | None] = mapped_column(String(80), index=True)
    external_reference: Mapped[str | None] = mapped_column(String(120), index=True)
    checkout_url: Mapped[str | None] = mapped_column(Text)
    pix_copy_paste: Mapped[str | None] = mapped_column(Text)
    pix_qr_code: Mapped[str | None] = mapped_column(Text)
    checkout_status: Mapped[str | None] = mapped_column(String(40))
    checkout_mode: Mapped[str | None] = mapped_column(String(20))
