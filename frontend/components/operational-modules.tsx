...
function ClientProfileFields({prefix,values,onChange}:{prefix:string;values:Record<string,string>;onChange:(k:string,v:string)=>void}) {
  return <div className="marketplace-profile-fields">
    <label className="marketplace-field">Renda mensal (R$)<CurrencyInput value={values[`${prefix}_income`]} onChange={v=>onChange(`${prefix}_income`,v)} required/></label>
...
export function ProposalsModule() {
  ...
  const available=useMemo(()=>quotas.filter(q=>q.status==="AVAILABLE"||q.status==="RESERVED"),[quotas]);
  async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();const form=e.currentTarget;const fd=new FormData(form);if(!requestedAmount)return;await api("/proposals",{method:"POST",body:JSON.stringify({lead_id:fd.get("lead_id"),product:fd.get("product"),requested_amount:requestedAmount,terms:{channel:"DASHBOARD"}})});form.reset();setRequestedAmount("");load()}
  ...
  return <OperationalLayout title="Propostas e simulações" subtitle="Cadastro comercial unificado para parceiros: Marketplace, SDC e Flash Capital. Simule, gere contrato e registre a venda." icon={<FileText/>}>
    ...
    <form className="quick-form" onSubmit={submit}><select name="lead_id" required>{leads.map(l=><option value={l.id} key={l.id}>{l.name}</option>)}</select><select name="product"><option value="MARKETPLACE">Marketplace</option><option value="SDC">SDC</option><option value="FLASH_CREDIT">Flash Capital</option></select><CurrencyInput value={requestedAmount} onChange={setRequestedAmount} placeholder="Valor solicitado (R$)" required/><button><Plus/>Nova proposta</button></form>
    ...
    {notice&&<div className="notice"><RefreshCw/>{notice}</div>}{lastCalculation&&<CalculationResult calculation={lastCalculation}/>}<DataTable headers={["Produto","Valor","Parceiro","Versão","Status","Workflow"]}>{items.map(p=>{const hasContract=contracts.some(c=>c.proposal_id===p.id);const needsQuota=p.product!=="FLASH_CREDIT";return <tr key={p.id}><td><b>{productLabel(p.product)}</b><small>{p.lead_name??leads.find(l=>l.id===p.lead_id)?.name}</small></td><td>{brl.format(Number(p.requested_amount))}</td><td><b>{p.owner_name??"—"}</b><small>{p.owner_role??""}</small></td><td>{p.calculation_version}</td><td><Pill value={p.status}/></td><td className="actions-cell"><button className="table-action" disabled={needsQuota&&!selected.length} onClick={()=>calculate(p)}>Simular</button><button className="table-action" disabled={hasContract} onClick={()=>contract(p)}>{hasContract?"Contrato criado":"Gerar contrato"}</button></td></tr>})}</DataTable>
  </OperationalLayout>
}
