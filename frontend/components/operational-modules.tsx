...
import { Administrator, api, Calculation, CommercialClient, Contract, Lead, Proposal, Quota, Reservation, User } from "@/lib/api";
...
export function InventoryModule() {
  ...
  return <OperationalLayout title="Inventário (admin)" subtitle="Cadastro interno de cotas, varredura Nina e trava de 60 min — estrutura operacional, não é a jornada comercial do parceiro." icon={<WalletCards/>}>
    <div className="notice"><Clock3/>Fluxo: <b>1.</b> Cadastro (admin) · <b>2.</b> Varredura Nina (regras internas da administradora) · <b>3.</b> Trava 60 min · <b>4.</b> Proposta em SDC/Marketplace · <b>5.</b> Contrato registra a venda · Fornecedores em <b>Fornecedores</b> · Ofertas públicas em <b>Vender minha cota</b></div>
...
function ClientProfileFields({prefix,values,flags,onChange,onFlag}:{prefix:string;values:Record<string,string>;flags:Record<string,boolean>;onChange:(k:string,v:string)=>void;onFlag:(k:string,v:boolean)=>void}) {
...
export function ProposalsModule() {
  const [items,setItems]=useState<Proposal[]>([]);const [leads,setLeads]=useState<Lead[]>([]);const [quotas,setQuotas]=useState<Quota[]>([]);const [contracts,setContracts]=useState<Contract[]>([]);const [clients,setClients]=useState<CommercialClient[]>([]);const [isCommercial,setIsCommercial]=useState(false);const [selected,setSelected]=useState<string[]>([]);const [notice,setNotice]=useState("");
  ...
  const available=useMemo(()=>quotas.filter(q=>q.status==="AVAILABLE"||q.status==="RESERVED"),[quotas]);
  async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();const form=e.currentTarget;const fd=new FormData(form);if(!requestedAmount)return;const clientId=String(fd.get("client_user_id")||"");await api("/proposals",{method:"POST",body:JSON.stringify({lead_id:fd.get("lead_id")||undefined,product:fd.get("product"),requested_amount:requestedAmount,client_user_id:clientId||undefined,sale_channel:isCommercial?"PARTNER_OFFICE":undefined,terms:{channel:isCommercial?"PARTNER_OFFICE":"SELF_SERVICE"}})});form.reset();setRequestedAmount("");load()}
